...
public class Principal {
    ...
    public static void main(String[] args){
        ...
        try {
            ...
            System.out.println("No se han podido cargar los contactos correctamente");
        }
...
//                    break;
    ...
    private static void oldCargarContactos() throws ParseException{
    ...
    private static void cargarContactos() throws FileNotFoundException, ParseException {
        
        Scanner lectorFichero = new Scanner(new FileReader("registro.txt"));
        String linea, strTipo;
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
        int id;
        ...
        while (lectorFichero.hasNextLine()){
            linea = lectorFichero.nextLine();
            String strContacto[]= linea.split("#");
            
            strTipo = strContacto[4].toUpperCase();
            
            miAgenda.firstAddContacto(new Contacto(Integer.parseInt(strContacto[0]),
                    strContacto[1], sdf.parse(strContacto[2]),
                    Long.parseLong(strContacto[3]), Tipo.valueOf(strTipo),
                    Boolean.parseBoolean(strContacto[5])), id);
...
//    }
    ...
    private static void guardarContactos() throws IOException{
        Contacto[] todosLosContactos = miAgenda.getAllContactos();
        Contacto registrarContacto;
        
            FileWriter escritor = new FileWriter("registro.txt");
            
            String registraID = miAgenda.getSiguienteID() + "\n";
            escritor.write(registraID);
            for (int i = 0; i < todosLosContactos.length; i++) {
                registrarContacto = todosLosContactos[i];
                
                escritor.write(registrarContacto.toRegistro(registrarContacto.getId(), 
                        registrarContacto.getNombre(), registrarContacto.getFechaNacimiento(),
                        registrarContacto.getTelefono(), registrarContacto.getTipo(),
                        registrarContacto.isTieneCoche()));
